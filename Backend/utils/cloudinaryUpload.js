 ...
 * @param {String} folder - Cloudinary folder path (e.g., 'Homebuddy24/documents')
 * @param {String} resourceType - 'image', 'raw', 'video', 'auto'
 * @returns {Promise<Object>} - Cloudinary upload result
 */
const uploadToCloudinary = (fileBuffer, folder = 'Homebuddy24/documents', resourceType = 'auto') => {
...
const uploadVendorDocument = async (fileBuffer, documentType, vendorId) => {
  try {
    const folder = `Homebuddy24/documents/vendors/${vendorId}`;
...
const uploadWorkerDocument = async (fileBuffer, workerId) => {
  try {
    const folder = `Homebuddy24/documents/workers/${workerId}`;
    const result = await uploadToCloudinary(fileBuffer, folder, 'auto');
    return result.secure_url;
  } catch (error) {
...
const uploadProfilePhoto = async (fileBuffer, userType, userId) => {
  try {
    const folder = `Homebuddy24/profiles/${userType}s/${userId}`;
...
const uploadPaymentScreenshot = async (base64Data, transactionId) => {
  try {
    const folder = `Homebuddy24/payment_proofs/${transactionId}`;
