...
const uploadHomeContentImages = async () => {
  try {
      ...
      for (const imagePath of section.images) {
        ...
        try {
          ...
          const result = await uploadFile(imageBuffer, {
            folder: `Homebuddy24/HomeContent/${section.name}`,
    ...
    const outputPath = path.join(process.cwd(), 'Homebuddy24-home-content-urls.json');
...
const runUpload = async () => {
  try {
    ...
    console.log('\n🎯 Next step: Update seeding scripts with these Homebuddy24 HomeContent URLs');
  } catch (error) {
