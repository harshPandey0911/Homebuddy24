...
const uploadCategoryImages = async () => {
  try {
    ...
    for (const category of categories) {
      ...
      // Upload category icon to Homebuddy24/{category}/icons/
      if (fs.existsSync(category.icon)) {
        ...
        const iconResult = await uploadFile(iconBuffer, {
          folder: `Homebuddy24/${category.name}/icons`,
    ...
    const outputPath = path.join(process.cwd(), 'Homebuddy24-category-icons.json');
...
const runUpload = async () => {
  try {
    ...
    console.log('\n🎯 Next step: Update seeding scripts with these Homebuddy24 Cloudinary URLs');
  } catch (error) {
